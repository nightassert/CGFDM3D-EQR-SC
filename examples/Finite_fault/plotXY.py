...
#   Discription: Plot the snapshot of X-Y plane
#
#	Update: Tianhong Xu, 12231218@mail.sustech.edu.cn
#   Update Time: 2023-11-16
#   Update Content: Add command line parameters
#
#   Reference:
...
for mpiY in range(grid.PY):
    for mpiX in range(grid.PX):
        mpiZ = mpiSliceZ
        XFile = open("%s_Z_mpi_%d_%d_%d.bin" %
                     (fileNameX, mpiX, mpiY, mpiZ), "rb")
        YFile = open("%s_Z_mpi_%d_%d_%d.bin" %
...
filename = varname + "_" + str(scheme) + ".png"
plt.savefig(filename, dpi=300)


line_Y = [100, 200, 300, 400, 500]
line_Y_data = np.zeros([len(line_Y), 401])
for i in range(len(line_Y)):
    print(dataY[line_Y[i], 0])
for i in range(len(line_Y)):
    line_Y_data[i, :] = data[line_Y[i], :]
np.savetxt(data_dir + "line_Y_data.txt", line_Y_data)
