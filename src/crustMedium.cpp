...
void crustInterpFunc(PARAMS params, GRID grid, int nLon, int nLat, int nLayer,
					 ...
					 LONLAT LonLat, float *coord, MPI_COORD thisMPICoord)
{
...
void solveStructure(GRID grid, float *structure, float *coord, CRUST crustInterp, int nLayer)
{
	int _nx_ = grid._nx_;
	int _ny_ = grid._ny_;
	int _nz_ = grid._nz_;

	long long index = 0, indexC = 0,
			  posl = 0, posl_1 = 0,
			  posnLayer_1 = 0,
			  pos0, posm, posm1;
	...
	structure[index + 0] = vs * CUnit;	// LAM;
	structure[index + 1] = vp * CUnit;	// MU;
	structure[index + 2] = rho * CUnit; // RHO;
...
void readCrustal_1(PARAMS params, GRID grid, MPI_COORD thisMPICoord, float *coord, float *structure)
{
	...
	char crustTxt[246];
	sprintf(crustTxt, "%s/crustTxt.txt", params.crustDir);
	...
	projTrans(lon_0, lat_0, grid, coord, LonLat, -180, 180, -90, 90);
