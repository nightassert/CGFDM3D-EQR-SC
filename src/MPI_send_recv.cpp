...
*   Discription: MPI send and receive data
*
*   Reference:
				 ...
				 int xStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, PACK_UNPACK_FUNC pack_unpack_func)
{
	...
	pack_unpack_func<<<blocks, threads>>>(con, thisSendRecv, xStartHalo, X0, Y0, Z0, X, Y, Z);
	CHECK(cudaDeviceSynchronize());
#else
	pack_unpack_func(con, thisSendRecv, xStartHalo, X0, Y0, Z0, X, Y, Z);
#endif
}
				 ...
				 int yStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, PACK_UNPACK_FUNC pack_unpack_func)
{
	...
	blocks.y = (Y0 + threads.y - 1) / threads.y;
	blocks.z = (Z0 + threads.z - 1) / threads.z;
	pack_unpack_func<<<blocks, threads>>>(con, thisSendRecv, yStartHalo, X0, Y0, Z0, X, Y, Z);
	CHECK(cudaDeviceSynchronize());
#else
	pack_unpack_func(con, thisSendRecv, yStartHalo, X0, Y0, Z0, X, Y, Z);
#endif
}
				 ...
				 int zStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, PACK_UNPACK_FUNC pack_unpack_func)
{
	...
	blocks.y = (Y0 + threads.y - 1) / threads.y;
	blocks.z = (Z0 + threads.z - 1) / threads.z;
	pack_unpack_func<<<blocks, threads>>>(con, thisSendRecv, zStartHalo, X0, Y0, Z0, X, Y, Z);
	CHECK(cudaDeviceSynchronize());
#else
	pack_unpack_func(con, thisSendRecv, zStartHalo, X0, Y0, Z0, X, Y, Z);
#endif
}
...
void mpiSendRecv(MPI_Comm comm_cart, MPI_NEIGHBOR mpiNeighbor, GRID grid, float *con, SEND_RECV_DATA sr, int VARSIZE)
{
	...
	xStartHalo = nx;
	if (mpiNeighbor.X2 >= 0)
		PackUnpackX(con, thisXSend2, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, packX);
	...
	xStartHalo = 0;
	if (mpiNeighbor.X1 >= 0)
		PackUnpackX(con, thisXRecv1, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackX);

	xStartHalo = HALO;
	if (mpiNeighbor.X1 >= 0)
		PackUnpackX(con, thisXSend1, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, packX);
	...
	xStartHalo = _nx;
	if (mpiNeighbor.X2 >= 0)
		PackUnpackX(con, thisXRecv2, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackX);

	// y direction data exchange
	yStartHalo = ny;
	if (mpiNeighbor.Y2 >= 0)
		PackUnpackY(con, thisYSend2, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, packY);
	...
	yStartHalo = 0;
	if (mpiNeighbor.Y1 >= 0)
		PackUnpackY(con, thisYRecv1, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackY);

	yStartHalo = HALO;
	if (mpiNeighbor.Y1 >= 0)
		PackUnpackY(con, thisYSend1, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, packY);
	...
	yStartHalo = _ny;
	if (mpiNeighbor.Y2 >= 0)
		PackUnpackY(con, thisYRecv2, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackY);

	// z direction data exchange
	zStartHalo = nz;
	if (mpiNeighbor.Z2 >= 0)
		PackUnpackZ(con, thisZSend2, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, packZ);
	...
	zStartHalo = 0;
	if (mpiNeighbor.Z1 >= 0)
		PackUnpackZ(con, thisZRecv1, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackZ);

	zStartHalo = HALO;
	if (mpiNeighbor.Z1 >= 0)
		PackUnpackZ(con, thisZSend1, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, packZ);
	...
	zStartHalo = _nz;
	if (mpiNeighbor.Z2 >= 0)
		PackUnpackZ(con, thisZRecv2, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, unpackZ);
